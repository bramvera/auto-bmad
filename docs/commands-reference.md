...
For epic selection, the wizard accepts `recommended` or explicit epic numbers. Completed epics are excluded from the runnable plan. Wizard reset only rebuilds the Auto-BMAD plan from current sprint status; it is not a hard project startover.
