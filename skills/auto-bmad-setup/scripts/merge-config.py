...
def apply_legacy_defaults(answers: dict, legacy_core: dict, legacy_module: dict) -> dict:
    ...
    if legacy_core:
        ...
        filled_core.update(core)
        merged["core"] = filled_core

    if legacy_module:
        mod = merged.get("module", {})
        filled_mod = dict(legacy_module)
        filled_mod.update(mod)
...
def extract_module_metadata(module_yaml: dict) -> dict:
    ...
    meta["version"] = module_yaml.get("module_version")
...
def merge_config(
    ...
    for key in _CORE_USER_KEYS:
        if key in config:
            if verbose:
                print(f"Removing user-only key '{key}' from config", file=sys.stderr)
            del config[key]
    ...
    if verbose:
        print(f"Writing '{module_code}' section with keys: {list(module_section.keys())}", file=sys.stderr)
