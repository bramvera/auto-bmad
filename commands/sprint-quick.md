...
description: 'Quick mode: run all stories in an epic — 3 steps per story, retro at epic end (no TEA required)'
---
...
**IMPORTANT — BMAD skills may commit and squash internally.** `bmad-retrospective` often commits its own changes. Before running, check `git log --oneline -3` to see if it already handled the commit. If so, skip the squash below.
