---
name: 'auto-gds-sprint-quick'
description: 'Quick mode: run all GDS stories in an epic — 3 steps per story, E2E tests at epic end (no TEA required)'
---
...
**ARCHITECTURE NOTE:** The sprint coordinator runs each story step as a direct Task call — it does NOT delegate to `/auto-gds-story-quick`. This avoids context exhaustion from nested agents. Sprint → Step agent (2 levels only).
...
- **DO NOT** invoke `/auto-gds-story-quick` as a nested skill. Run the underlying BMAD skills directly to avoid nesting.
...
```markdown
# Sprint Progress: GDS Epic {{EPIC_ID}} — Quick Mode (LIVE)
...
- **Story 1-3** (failed at step 2 — Develop): Missing dependency
  ...
  - Recovery: `git reset --hard ghi9012`, fix the issue, then `/auto-gds-story-quick 1-3`
```
...
```markdown
# Sprint Report: GDS Epic {{EPIC_ID}} (Quick Mode)

| Field | Value |
|-------|-------|
| Pipeline | gds-sprint-quick |
| Mode | quick (3 steps per story) |
| Epic | {{EPIC_ID}} |
...
**IMPORTANT: Use dash syntax (e.g. `/auto-gds-sprint-quick`) NOT colon syntax when suggesting commands.**

### Next
- Review the sprint report and any failed stories
- If failed stories exist, run them individually: `/auto-gds-story-quick <story-id>`
- If more epics remain, run `/auto-gds-sprint-quick <next-epic-number>` for the next epic
