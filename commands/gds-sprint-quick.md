...
description: 'Quick mode: run all GDS stories in an epic — 3 steps per story, retro at epic end (no TEA required)'
---
...
Run the entire epic lifecycle in quick mode — no epic-start, 3 steps per story, retrospective at epic-end. Fully hands-off.
...
**IMPORTANT — BMAD skills may commit and squash internally.** `gds-retrospective` often commits its own changes. Before running, check `git log --oneline -3` to see if it already handled the commit. If so, skip the squash below.

After epic-end completes, check if the BMAD skill already squashed. If a commit like `chore(epic-X): epic end` already exists, skip the squash. Otherwise: `git reset --soft {{EPIC_END_START_COMMIT}}` then `git add -A && git commit -m "chore(epic-{{EPIC_ID}}): epic end — retro done"`
